...
const Views = {
  ...
  baccaratStartMenu: () => entryMenuHTML('🃏 Baccarat', [
    ...
    { icon: '💰', name: '페이아웃 연습하기', desc: '베팅 종류별<br>페이아웃 계산 연습', onclick: "App.navigate('baccarat','paysim')" },
  ]),
  ...
  blackjackStartMenu: () => entryMenuHTML('♠ Blackjack', [
    { icon: '♠', name: '일반모드', desc: '플레이어 진행부터<br>핸드 결과 판정까지 연습', onclick: "App.navigate('blackjack','simulation')" },
    { icon: '⚡', name: '스피드모드', desc: '플레이어 진행은 건너뛰고<br>딜러 드로우부터 연습', onclick: "App.navigate('blackjack','simulationspeed')" },
  ]),
  ...
  roulettePay: (() => {
    ...
    return {
      ...
      battle: {
        ...
        _onSnapshot(data) {
          if (!data) {
            // 방이 삭제됨 — 호스트가 나갔거나(어느 단계에서든 즉시 방을
            // 해체), 배틀 종료 후 참가자 전원이 나가 방이 비어 자동
            // 삭제된 경우. 어느 쪽이든 항상 메뉴로 돌려보낸다.
            this.teardown();
